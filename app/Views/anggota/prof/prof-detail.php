...
<!-- Page Header -->
<div class="mb-8">
  <h1 class="text-2xl font-bold text-slate-800"><?= $page_title ?? 'Profil Anggota' ?></h1>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div class="flex flex-col sm:flex-row items-center sm:items-end -mt-12 sm:-mt-10 gap-6">
        <div class="relative shrink-0">
          <div class="w-32 h-32 rounded-full border-4 border-white shadow-md overflow-hidden bg-white">
            ...
            // Fallback if image doesn't exist or string is empty
            $avatarSrc = (!empty($duser->profil_pic)) ? base_url($avatarPath) : 'https://ui-avatars.com/api/?name=' . urlencode($duser->nama_lengkap) . '&background=random';
            ?>
        ...
        <div class="flex-1 text-center sm:text-left mb-2">
          ...
          <p class="text-slate-500 font-medium"><?= $duser->username ?> &bull; Anggota</p>
    ...
    <div class="px-6 border-t border-slate-100">
      <nav class="flex space-x-6 overflow-x-auto scroller-none" id="profileTabs">
        <button onclick="switchTab('overview')" id="tab-overview" class="tab-btn active-tab py-4 text-sm font-semibold text-blue-600 border-b-2 border-blue-600 transition-colors whitespace-nowrap">
          Detail Profil
        </button>
  ...
  <div class="tab-content relative">
    ...
    <div id="content-overview" class="tab-pane block fade-in">
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
          ...
          <div class="group">
            <label class="block text-xs uppercase tracking-wider text-slate-400 font-semibold mb-1">Status Kepegawaian</label>
            <div class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 uppercase">
              <?= $duser->status_pegawai ?>
            </div>
          </div>
          ...
          <div class="group">
            <label class="block text-xs uppercase tracking-wider text-slate-400 font-semibold mb-1">Bank & No. Rekening</label>
            <div class="text-base font-medium text-slate-800"><?= strtoupper($duser->nama_bank) ?> &mdash; <?= $duser->no_rek ?></div>
          </div>
    ...
    <div id="content-edit" class="tab-pane hidden">
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8">
        ...
        <form action="<?= url_to('anggota/profile/edit_proc') ?>" method="post" enctype="multipart/form-data" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" name="nama_lengkap" value="<?= $duser->nama_lengkap ?>" required
                class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition-all text-sm">
            </div>
            ...
            <div>
              ...
              <select name="instansi" required class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition-all text-sm">
            ...
            <div>
              ...
              <select name="status_pegawai" required class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition-all text-sm">
                <option value="" disabled <?= (!$duser->status_pegawai) ? 'selected' : '' ?>>Pilih...</option>
          ...
          <div>
            ...
            <textarea name="alamat" rows="3" required class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition-all text-sm"><?= $duser->alamat ?></textarea>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" name="nama_bank" value="<?= $duser->nama_bank ?>" required
                class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition-all text-sm">
            </div>
            <!-- No Rekening -->
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-2">Nomor Rekening <span class="text-red-500">*</span></label>
              <input type="number" name="no_rek" value="<?= $duser->no_rek ?>" required
                class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition-all text-sm">
            </div>
            <!-- Phone -->
            <div>
          ...
          <div class="border-t border-slate-100 pt-6 mt-6">
            <h4 class="text-sm font-bold text-slate-800 mb-4">Update Dokumen & Foto</h4>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Upload KTP</label>
                <input type="file" name="ktp_file" accept="image/jpg, image/jpeg, image/png, application/pdf"
                  class="block w-full text-sm text-slate-500
                                      file:mr-4 file:py-2 file:px-4
                                      file:rounded-full file:border-0
                                      file:text-sm file:font-semibold
                                      file:bg-blue-50 file:text-blue-700
                                      hover:file:bg-blue-100 transition-all">
                <p class="text-xs text-slate-400 mt-2">JPG, PNG, atau PDF. Maks 4MB. Kosongkan jika tidak ingin mengubah.</p>
              </div>
            </div>
          </div>

          <div class="pt-6 border-t border-slate-100 flex justify-end">
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 focus:ring-4 focus:ring-blue-200 transition-all transform active:scale-95 shadow-lg shadow-blue-600/30">
    ...
    <div id="content-password" class="tab-pane hidden">
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8">
        ...
        <form action="<?= url_to('anggota/profile/edit_pass') ?>" method="post" class="max-w-2xl">
          <div class="space-y-6">
            <!-- Old Pass -->
            <div>
              ...
              <div class="relative">
                <input type="password" name="old_pass" id="old_pass" required
                  class="w-full rounded-xl border-slate-200 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition-all text-sm pr-12">
