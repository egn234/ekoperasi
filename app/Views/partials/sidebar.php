        ...
        <nav class="flex-1 space-y-2">
            ...
            ?>
                <a href="<?= base_url($menu['url']) ?>"
                    class="w-full flex items-center space-x-4 px-6 py-4 rounded-2xl transition-all font-black text-xs uppercase tracking-widest relative group overflow-hidden
               <?= $isActive ? 'bg-blue-gradient text-white shadow-xl shadow-blue-200/50 scale-[1.02]' : 'text-slate-400 hover:text-blue-600 hover:bg-slate-50' ?>">
                    ...
                    <i data-lucide="<?= $menu['icon'] ?>" class="w-5 h-5 <?= $isActive ? 'text-white' : 'group-hover:text-blue-600' ?>"></i>
        ...
        <div class="mt-8 pt-8 border-t border-slate-100">
            <div class="flex items-center space-x-3 mb-6 px-2">
                <div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-bold overflow-hidden border-2 border-white shadow-md">
                    <img src="<?= base_url('assets/images/users/avatar-1.jpg') ?>" alt="User" class="w-full h-full object-cover">
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-bold text-slate-900 truncate"><?= session()->get('username') ?? 'User' ?></p>
            ...
            <a href="<?= base_url('logout') ?>"
                class="w-full flex items-center justify-center space-x-4 px-6 py-4 rounded-2xl text-red-500 bg-red-50/50 font-black text-xs uppercase tracking-widest hover:bg-red-500 hover:text-white hover:shadow-lg hover:shadow-red-200 transition-all duration-300 group">
