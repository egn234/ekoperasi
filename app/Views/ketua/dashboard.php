...
<div class="space-y-8">
  ...
  <div class="bg-indigo-gradient rounded-[2rem] p-8 md:p-10 relative overflow-hidden shadow-xl shadow-indigo-200/50 text-white">
    ...
    <div class="absolute top-0 right-0 p-4 opacity-10 pointer-events-none">
      <i data-lucide="crown" class="w-64 h-64"></i>
    </div>

    <div class="relative z-10">
      <h1 class="text-3xl font-black tracking-tight mb-2">Dashboard Ketua</h1>
      <p class="font-medium text-indigo-100 max-w-2xl">
        Pantau kinerja koperasi secara menyeluruh. Ringkasan keuangan dan operasional terkini.
      </p>
    </div>
  </div>
  ...
  <div class="bg-white rounded-[2.5rem] p-8 shadow-soft border border-slate-100">
    ...
    <div class="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-slate-100">
      ...
      <div class="text-center">
        ...
        <p id="averageValue" class="text-lg font-black text-indigo-600">-</p>
      </div>
    </div>
  </div>
  ...
  <div class="bg-white rounded-[2.5rem] p-8 shadow-soft border border-slate-100">
    <div class="flex justify-between items-center mb-8">
      <div>
        ...
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Daftar Pengajuan Terbaru</p>
      </div>
    </div>
...
<script>
  async function openModal(type, id) {
    ...
    contentArea.innerHTML = '<div class="text-center py-12"><div class="w-10 h-10 border-4 border-slate-100 border-t-indigo-600 rounded-full animate-spin mx-auto"></div></div>';
    ...
    try {
      ...
      contentArea.innerHTML = '<p class="text-red-500 text-center font-bold">Gagal memuat data.</p>';
    }
  }
