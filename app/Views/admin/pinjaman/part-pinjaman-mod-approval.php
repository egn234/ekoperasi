      ...
      <div class="mb-3">
        ...
        <?php if (in_array(pathinfo($b->ktp_file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png'])): ?>
          <div class="text-center">
            <img src="<?= base_url('uploads/user/' . $b->username . '/ktp/' . $b->ktp_file) ?>" alt="KTP Preview" class="img-fluid" style="max-height: 200px;">
          </div>
        <?php elseif (pathinfo($b->ktp_file, PATHINFO_EXTENSION) === 'pdf'): ?>
          <embed src="<?= base_url('uploads/user/' . $b->username . '/ktp/' . $b->ktp_file) ?>" type="application/pdf" width="100%" height="400px">
        <?php endif; ?>
      </div>
    <?php else: ?>
      <div class="mb-3">
        <label class="form-label">Preview KTP</label>
        <p class="text-muted">Tidak ada file KTP yang diunggah.</p>
      </div>
    <?php endif; ?>
    <!-- End KTP File Preview -->
