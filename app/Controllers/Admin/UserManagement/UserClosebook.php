    ...
    public function flag_switch($iduser = false)
    {
        ...
        if ($user->user_flag == 0) {
            ...
            if ($user->closebook_param_count == 1) {
                ...
                // Validasi: Cek apakah sudah 1 tahun sejak closebook terakhir
                if ($user->closebook_last_updated) {
                    $last_update = strtotime($user->closebook_last_updated);
                    $one_year_later = strtotime('+1 year', $last_update);
                    $now = time();

                    if ($now < $one_year_later) {
                        $remaining_days = ceil(($one_year_later - $now) / (60 * 60 * 24));
                        $alert = view('partials/notification-alert', [
                            'notif_text' => 'User belum dapat diaktifkan kembali. Harus menunggu 1 tahun sejak penonaktifan terakhir (' . date('d F Y', $last_update) . '). Sisa waktu: ' . $remaining_days . ' hari.',
                            'status' => 'warning'
                        ]);
